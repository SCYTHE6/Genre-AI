...
const USE_GENRE_EFFECTS = true; // Toggle between genre effects and Spleeter

// Function to process audio using Spleeter
export async function transformAudio(
  ...
  try {
    ...
    const scriptPath = USE_GENRE_EFFECTS 
      ? path.join(process.cwd(), 'ml_scripts', 'magenta_inspired.py')
...
function createSimpleTransformScript() {
  // The script content would go here - it's quite long so I'm omitting it
  // You should paste the script from Step 4 here as a string
  return `import sys
import os
import numpy as np
import librosa
